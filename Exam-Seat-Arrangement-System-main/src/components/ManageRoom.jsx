...
export default function ManageRoom() {
    ...
    const filteredRows = useMemo(() => {
        ...
        if (list.length > 0 && filterTerm) {
            if (filterTerm === 'old-building' || filterTerm === 'new-building') {
        ...
        const postRooms = async () => {
            ...
            try {
                ...
                if (error.response?.status === 409) {
                    setError("Room number already exists. Please use a different room number.");
                } else {
        ...
        <div className="bg-background flex flex-col flex-grow md:w-5/6">
            <div className="px-8 pt-4 mt-6">
                ...
                <form ref={formRef} className="flex hw:flex-col flex-row justify-between" onSubmit={handleRoom}>
                    ...
                    <DropDownInput id="slot" title="Block" inputRef={blockRef} options={['new-building', 'old-building']} required />
                ...
                <div className="h-72 overflow-y-auto relative">
                    <table className="table-auto w-full">
                        ...
                        <tbody>
                            ...
                            ) : (list.map(item => <Row key={item.room_no} room={item.room_no} floor={item.floor_no} block={item.block}
